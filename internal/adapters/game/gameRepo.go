...
// func stringToCardMap(Map string) map[string]card.Card {
// 	m := stringToStringMap(Map)
// 	a := make(map[string]card.Card)
// 	for k, v := range m {
// 		a[k] = card.Card.MakeCardFromString(v)
// 	}
// 	return a
// }
...
func (repo *GameRepository) CreateNewGame(g *game.Game) error {
	sqlCode := `
	INSERT INTO games (game_id, bot_id, started, ended, winner, history, config)
	VALUES ($1, $2, $3, $4, $5, $6, $7)`
	// TODO: Add config
	_, err := repo.db.Exec(sqlCode, g.GameID, pq.Array(g.BotID), g.Started, g.Ended, g.Winner, "{}", "")
	return err
}

func (repo *GameRepository) GetGameByID(id string) (*game.Game, error) {
	sqlCode := `SELECT * FROM games WHERE game_id=$1;`
	var game game.Game
	row := repo.db.QueryRow(sqlCode, id)
	// TODO: Add config
	var test string
	err := row.Scan(&game.GameID, pq.Array(&game.BotID), &game.Started, &game.Ended, &game.Winner, pq.Array(&game.History), &test)
	if err == nil {
		return &game, nil
	} else if err == sql.ErrNoRows {
		return nil, errors.New("no id found")
